  ...
  const result = useQuery(["TL_APPLICATIONS_LIST", params], useTLSearch(params, config), {
    ...
    select: (data) => {
      return data.map((i) => ({
        ...
        TL_COMMON_TABLE_COL_OWN_NAME: i?.tradeLicenseDetail?.licenseeType?.includes("INSTITUTION")
        ...
        TL_INSTITUTION_TYPE_LABEL: i?.tradeLicenseDetail?.licenseeType?.includes("INSTITUTION")
          ? `TL_${i?.tradeLicenseDetail?.licenseeType}`
          : null,
        raw: i,
      }));
    },
